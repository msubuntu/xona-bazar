...
import { notifyChatMessage } from '../services/telegramBot.js'
  ...
  try {
    ...
    conversation.participants.forEach(pid => {
      ...
      if (uid !== req.user._id.toString()) {
        notifyChatMessage(uid, req.user, conversation._id, text)
      }
    })
...
router.post('/:id/messages', protect, async (req, res) => {
  try {
    ...
    conversation.participants.forEach(pid => {
      const pidStr = pid.toString()
      if (pidStr !== uid) {
        notifyChatMessage(pidStr, req.user, conversation._id, text)
      }
    })
